...
if( ! function_exists( 'shortcode_boc_button' ) ) {
	function shortcode_boc_button($atts, $content = null){
		...
		return	'<a	href="'.esc_url($href).'" 
			class="button '.esc_attr($size.' '.$color.' '.$border_radius.' '.$btn_style.' '.$icon_pos.' '.$icon_effect.' '.$css_animation_classes.' '.$css_classes).'" '.wp_kses_post($target).'>'.
				(($icon_pos=='icon_pos_before') ? wp_kses_post($icon) : "").'<span>'.do_shortcode(esc_html($btn_content)).'</span>'.(($icon_pos=='icon_pos_after') ? wp_kses_post($icon) : "").'</a>';  
	}
