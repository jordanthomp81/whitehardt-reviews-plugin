...
function boc_styles() {
	...
	// Load Main CSS
	wp_enqueue_style( 'boc-main-styles', get_bloginfo( 'stylesheet_url' ) );
	...
	wp_add_inline_style( 'boc-main-styles', $inline_css );
}
...
function boc_scripts() {
	...
	if(ot_get_option('retina','off') == 'on'){
		wp_enqueue_script('retina_js', get_template_directory_uri().'/js/retina.min.js' );
