...
if( ! function_exists( 'boc_breadcrumbs' ) ) { 
	function boc_breadcrumbs() {
			...
			echo '<div class="breadcrumb '.($breadcrumbs_position!="normal" ? "breadcrumb_right" : "") .'">';
			...
			if(function_exists('is_woocommerce') && is_woocommerce()) {
				...
				if(function_exists('is_shop') && is_shop()) {
					echo "<a class='last_bc' href='". esc_url(get_permalink( woocommerce_get_page_id( 'shop' ) ))."'><span>".__('Shop', 'Savia')."</span></a>";
				}elseif(is_product()){
					echo "<a href='". esc_url(get_permalink( woocommerce_get_page_id( 'shop' ) ))."'><span>".__('Shop', 'Savia')."</span></a>";
	...
	function boc_search_form_in_header() {
		...
		$search_form_html = '
			<div id="boc_searchform_in_header" class="">
				<div class="container">
					<form method="get" action="'.esc_url( home_url( '/' ) ).'" role="search" class="header_search_form"><input type="search" name="s" autocomplete="off" placeholder="'.__( 'Type then hit enter to search...', 'Fortuna' ).'" /></form>
			...
			<div class="single_comment">
				...
				<div class="comment_content">
					...
					<div class="comment-author meta">
						<div class="comment_name"><?php echo wp_kses_post(get_comment_author_link()); ?><span>-</span><?php esc_url(comment_reply_link(array_merge( $args, array('reply_text' => 'Reply', 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'])))); ?></div>
...
if( ! function_exists( 'boc_get_BGR_css' ) ) {
	function boc_get_BGR_css(){
		...
		if ( is_single() || is_page() ) {
			...
			$page_heading_bgr_meta = get_post_meta( get_the_ID(), 'boc_page_heading_bgr_meta', true );
			...
			if(get_post_meta( get_the_ID(), 'boc_page_heading_white_text_meta', true ) == 'on'){
				$custom_background_css .= 
...
if( ! function_exists( 'boc_get_inline_CSS' ) ) { 
	function boc_get_inline_CSS() {
		...
		$fonts_available = 	get_theme_mod( 'ot_google_fonts', array() );
		...
		if($boc_main_color != $boc_default_color){
			...
			$inline_css .='	
				...
				.post_content a:not(.button):visited {	color: '.$boc_main_color.'; }
			...
			$inline_css .='	
				@media only screen and (min-width: 1018px){
					...
					#header.transparent_header:not(.scrolled) #subheader a:hover { 
						color:'.esc_attr(boc_get_transparent_header_nav_hover_color()).'; }
					...
					#header.transparent_header:not(.scrolled) #menu > ul > li.boc_nav_button a:hover	{
						background:'.esc_attr(boc_get_transparent_header_nav_hover_color()).';
						border: 2px solid '.esc_attr(boc_get_transparent_header_nav_hover_color()).';
					}
				}				
			';
		}
